...
// [[Rcpp::export]]
double obj0cpp_mix(vec w, vec beta, vec mu, double rho, double alpha, double rho1,
  ...
  double h = trexpm(pp) - n_nodes;
  // double h = trace(expmat(pp)) - n_nodes;
  vec vector = w - beta + mu;

  double result = (loss_conti + loss_multi + loss_ordin) +
    (rho1/2.0) * pow(h, 2) + alpha * h + (rho/2.0) * pow(norm(vector, 2), 2);
