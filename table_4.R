#######################################################
## Table 4 (Large Graphs, Nom vs Ord, Results) ####
#######################################################
...
if(T){ # for debugging, delete when submitting
  ...
  save_file_name = paste0("./results/simu2_large_results/simu2_large_",
...
if(F){ # for server running
  ...
  method = as.character(queue_args[2])
  iter = as.integer(queue_args[3])
  save_file_name = paste0("simu2_large_",graph_type,"_",method,"_",iter,".rds")
...
result = glmdag(data_input, n_lams = n_lams, verbose = T,
                admm_args = admm_arg_ctrl(warm_start = T))
...
# subdir = paste0("./results/", simu_title, "/")
# methods =  c("mc","mm","mo")
